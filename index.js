...
function findElementGreaterThan(arr, num) {
  ...
  // Використовуємо метод find для пошуку першого елементу, який є більшим за задане число num.
   const arr1 = arr.find(element => element > num);
   return arr1;
...
function findLastEvenNumber(arr) {
  ...
  // За допомогою метода findLast знаходимо останнє парне число в масиві.
    const arrLastIn = arr.findLast((element) => element = element % 2 === 0);
  // Повертаємо знайдене число або 'Парних чисел не знайдено', якщо таке число не знайдено.
  return arrLastIn || "Парних чисел не згайдено";
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArrayString = arr1.join(" ")
  // Повертаємо рядок.
...
function getArrayKeys(arr) {
  ...
  const arr1 = Array.from(iterator);
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = arr1.filter(num => num > 0)
...
function sumAndPrint(arr) {
  ...
  // За допомогою методу reduce() обчислюємо суму елементів масиву.
    const sum = arr.reduce((num, arr) => num + arr, 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const filteredSquaredArray = map1.filter(num => num > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sumLastElm = arr1.reduceRight((num, arr1) => num + arr1, 0);
  // console.log(sumLastElm);
...
function expandArrayByN(arr, n) {
  ...
  const newArr = arr.flatMap((n) =>  [n, n, n]); 
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((accumulator, currentValue) => (accumulator.length > currentValue.length) ? accumulator : currentValue);
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((element, index, arr) => arr.indexOf(element) !== index);
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((num, price) => num + price.price, 0)
